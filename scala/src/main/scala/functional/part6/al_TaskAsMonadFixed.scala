...
object al_TaskAsMonadFixed {
  ...
  given Monad[Task] with {
    ...
    def flatMap[A, B](ma: Task[A], f: A => Task[B]): Task[B] = Task(
      (callbackB: Callback[B], ec: ExecutionContext) => {
        // It's not clear if the first ma task should be executed sync or async, here we choose async
    ...
    // IMPORTANT: Although we get an implementation of ap from the Monad definition, it will be implemented
    // using flatMap which is by definition works sequentially and not parallelly, so to enjoy the parallel nature
    // of the Applicative Functor combinators we need to write a different implementation
  ...
  def main(args: Array[String]): Unit = {
    ...
    // This looks very similar to the special async/await syntax we have in python/JS
