...
object ai_TaskAsApplicativeSequential {
  ...
  given Applicative[Task] with {
    ...
    override def ap[A, B](fa: Task[A], ff: Task[A => B]): Task[B] = Task {
      (callbackB: Callback[B], ec: ExecutionContext) => {
        ff.execute((fr: AsyncResult[A => B]) => fr match {
          case Success(f) =>
            fa.execute((fa: AsyncResult[A]) => fa match {
              case Success(a) => callbackB(Success(f(a)))
  ...
  def main(args: Array[String]): Unit = {
    ...
    val operations = List(operation(1), operation(2), operation(3))
