...
from typing import Any, Callable, Never, Sequence, cast, override
...
type IOYield[E, D] = StopFromError[E] | Gather | Sleep | RequestDependency[D] | Nop
...
class _ProvideDep[E, A, D, DS]:
    ...
    # iterator protocol
    def __next__(self) -> IOYield[E, DS]:
        return self.send(None)

    def __iter__(self) -> IO[E, A, DS]:
        return self


def provide[E, A, D, DS](
    dep_type: type[D], remaining_types: TypeForm[DS], dep: D, io: IO[E, A, D | DS]
) -> IO[E, A, DS]:
    return _ProvideDep(dep_type, dep, io)
...
def prog1() -> IO[OutOfSpace, tuple[int, int], StateDao | Logger | SomeAdditionalDep]:
    ...
    other_dep = yield from get_dependency(SomeAdditionalDep)
    logger.info(f"End prog1 - {other_dep}")
    return r1, r2
...
def main() -> None:
    # NOTE: this will not compile the dependency resolver, resolver doesn't know how to resolve SomeAdditionalDep
    # r1 = run_io(prog1(), DependencyResolverImpl())

    # NOTE: annoying syntax since we don't have Exclude[D, DS], need to separate the list of dependecies manually
    partly_resolved = provide(SomeAdditionalDep, StateDao | Logger, SomeAdditionalDep("got it"), prog1())

    # NOTE: if we had a nice syntax for provide, could have written the interpreter to only accepts IO without any deps
    #       forcing us to provide all deps before calling the interpreter
    r1 = run_io(partly_resolved, DependencyResolverImpl())
    print(f"{r1=}")
